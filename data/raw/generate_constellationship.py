...
def snt_data():
    ...
    data_regex = re.compile(
        r"([0-9\. -]{5}) ([0-9\. ]{8}) ([0-9\. ]{8}) ([A-Za-z0-9 -]{3})([a-zA-Z0-9 ])([0-9])([a-zA-Z]{3})"
    )
    for line in sys.stdin:
        ...
        if m:
            ...
            if not line.startswith("#"):
                print(
                    "WARNING: No match: {}".format(line), file=sys.stderr
...
if __name__ == "__main__":
    ...
    try:
        for vertex in snt_data():
            ...
            print(
                "{bayer} {constellation} [ra={ra}, dec={dec}, mag={mag}]...".format(
                    **vertex
                ),
                file=sys.stderr,
                end="",
            ...
            if vertex["constellation"] == "Cyg" and vertex["bayer"] == "31-":
                ...
                current_hip = get_hip(
                    ra=vertex["ra"], dec=vertex["dec"], mag=vertex["mag"]
                )
            if not current_hip:
                raise ValueError(
            ...
            if previous_line == current_line and previous_hip and current_hip:
                constellationship[vertex["constellation"]].append(
                    (
                        previous_hip,
                        current_hip,
                    )
    ...
    constellationship["Men"].append(
        (
            29271,
            23467,
        )
    )
    constellationship["Mic"].append(
        (
            102831,
            102989,
        )
    ...
    for constellation in sorted(constellationship.keys()):
        ...
        print(
            "{} {} {}".format(
                constellation,
                len(constellationship[constellation]),
                " ".join(
                    [
                        "{} {}".format(i[0], i[1])
                        for i in constellationship[constellation]
                    ]
                ),
            )
        )
